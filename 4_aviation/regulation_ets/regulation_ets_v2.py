...
df_emissions_proj = pd.read_excel(
    io = 'data/data.xlsx',
...
df_emissions_allowances = pd.read_excel(
    io = 'data/data.xlsx',
    sheet_name = 'ETS (current)',
    usecols = lambda column: column in [
        'year',
        'allowances [% of 2018 value]',
        'free allowances [% of all allowances]'
    ],
    dtype={
        'year': int,
        'allowances [% of 2018 value]': float,
        'free allowances [% of all allowances]': float
    },
    header = 0,
    engine = 'openpyxl'
)
...
def sum_selected_row_elements(df, row_index, columns):
    """
    Sums the elements of all cells corresponding to a row index and list of column names.
    """
...
allowances_2018 = sum_selected_row_elements(
    df = df_emissions_hist,
...
df_emissions_allowances['all allowances [t(CO2)]'] = df_emissions_allowances['allowances [% of 2018 value]']/100 * allowances_2018

df_emissions_allowances['free allowances [t(CO2)]'] = df_emissions_allowances['all allowances [t(CO2)]'] * 0.82 * df_emissions_allowances['free allowances [% of all allowances]']/100

df_emissions_allowances['special reserve [t(CO2)]'] = df_emissions_allowances['all allowances [t(CO2)]']/100 * 0.03

df_emissions_allowances['state auction [t(CO2)]'] = df_emissions_allowances['all allowances [t(CO2)]'] - df_emissions_allowances['free allowances [t(CO2)]'] - df_emissions_allowances['special reserve [t(CO2)]']

df_emissions_proj_filtered = df_emissions_proj[df_emissions_proj['year'] >= 2021].reset_index()
df_emissions_allowances['EUA purchases [t(CO2)]'] = df_emissions_proj_filtered['proj'] - df_emissions_allowances['all allowances [t(CO2)]']
